...
# -N/procs: Number of processors to use in bowtie.
';

##############################################################################################################################
## PROCESS INPUTS

my( $INPUT, $TARGETS, $LIB_A, $LIB_D, @STEPS, $fileBase, $barLength, $barOffset,$A_length,$D_length,$VERBOSE, @QUIET,$PAIRS,$bowtieProcs ) =
  ( undef,   undef,   undef,    undef,  undef, undef, undef, 3, 20, 15, 1,undef, undef,5 );
$barOffset = 3;
$A_length = 20;
$D_length = 15;
$VERBOSE = 1;
...
OPTS: {
  ...
  while( my( $option, $value, $pretty ) = Getopt::Mixed::nextOption() ) {
        ...
        $bowtieProcs = $value if $option eq 'N';     
  }
...
foreach $step (@STEPS){
  switch ($step){     # Step 1: parse/decode
    ...
    case 1 {
      ...
      ## Set up command:
      $args = "perl /home/jeremy/RASL/parseRASL2.pl \"$INPUT\" \"$TARGETS\" \"$fileBase\" $D_length";
    ...
    case 2 {    # Step 2: run bowtie (on each file)
        ...
        for(my $i=0;$i < scalar(@$samples);$i ++){          
          ...
          system($args) == 0  or die "System Command $args failed: $?";          
          $args = sprintf("bowtie -v 1 -m 1 --best -f -p %i \"%s\" \"%s\" \"%s\" 2>> \"%s\"",$bowtieProcs,  $LIB_D, $donorFiles->[$i] ,  $donors_aligned->[$i],$logFile);          
          print "Running command: ----> $args \n\n" if $VERBOSE;
          system($args) == 0  or die "System Command $args failed: $?";
          ...
          system($args) == 0  or die "System Command $args failed: $?";                        
          $args = sprintf("bowtie -v 1 -m 1 --best -f -p %i \"%s\" \"%s\" \"%s\" 2>> \"%s\"",$bowtieProcs, $LIB_A,  $acceptorFiles->[$i] ,  $acceptors_aligned->[$i],$logFile);
          print "Running command: ----> $args \n\n" if $VERBOSE;
          system($args) == 0  or die "System Command $`args failed: $?";
    ...
    case 3 {    # Step 3: Evaluate matches/mismatches (on each file)
        ...
        for(my $i=0;$i < scalar(@$samples);$i ++){                    
          ...
          $args = "perl /home/jeremy/RASL/matchAlignmentsToPairs.pl \"$sampleBase\" \"$PAIRS\"";
          print "Running command: ----> $args \n\n" if $VERBOSE;
          system($args) == 0  or die "System Command $args failed: $?" ;
        }
...
sub getSamples{
  ...
  while(defined($line= <INPUT_TARGETS>)){
  	...
  	chomp $line;
  	next if ($line =~ m/Seq/i || $line =~ m/Code/i);
